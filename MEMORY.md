...
- **TimeWeb** — 64.188.57.249 (сервер "Cute Vulpecula", ID 4662767) — крипто-бот Pamp-Damp + сервер VPN-бота `@djarvisvpn_bot`. Оплата ~16-го, напоминание 11-го. **15.08.2026: авария в ДЦ Equinix FR5 (Германия)** — TimeWeb сам подтвердил баннером в панели, расследование инцидента. SSH/443 недоступны отовсюду (проверено и напрямую, и через Fornex-туннель) — значит дело не в блокировке провайдером (версия от 14.08 была неверной), а в реальной аварии на стороне хостера. Оба сервиса (crypto-bot, vpn-бот) недоступны до починки ДЦ. Клиент s-ui `test-july` оставлен в базе.
