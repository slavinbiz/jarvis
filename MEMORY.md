...
- **TimeWeb** — 64.188.57.249 — крипто-бот Pamp-Damp. Оплата ~16-го, напоминание 11-го. **Личный VPN здесь больше не работает** — 14.08.2026 обнаружено, что провайдер заблокировал сам IP целиком (не только Hysteria2/UDP — не проходил даже SSH напрямую). Клиент s-ui `test-july` оставлен в базе, но бесполезен для личного использования с этой машины; VPN-бот для клиентов (другой проект, другие IP клиентов) не проверялся на этот же блок.
