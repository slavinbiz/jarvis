...
- **17.08.2026 — IP сервера заблокирован в РФ (провайдер/ТСПУ), прямой SSH/scp/ping с Windows-машины не проходит.** Сервер живой, подтверждено с Fornex и у самого TimeWeb (0% потерь, ttl 55-57). Работать с сервером теперь только через Fornex как jump host: `ssh -J root@213.193.196.125 root@64.188.57.249 "команда"`. Для scp — `scp -o ProxyJump=root@213.193.196.125 crypto_bot.py root@64.188.57.249:/root/crypto_bot.py` (тот же ключ `~/.ssh/id_ed25519` подходит на обоих серверах), либо в два шага через Fornex. Детали разбора — MEMORY.md, раздел «Инфраструктура» → TimeWeb
